...
public class Deck {
    ...
    private String getGameState(){
        ...
        if( (playerDecks.get(0).empty() && playerHands.get(0).isEmpty())
             || (playerDecks.get(1).empty() && playerHands.get(1).isEmpty()) )
        {
            if(playerDecks.get(0).empty())
            {
                player1Message = "Player1 Wins!";
                player2Message = "Player1 Wins!";
            }
            else
            {
                player1Message = "Player2 Wins!";
                player2Message = "Player2 Wins!";
            }
        }
    ...
    public boolean PlayACard(int playerIndex, int cardIndex, int playPileIndex){
        ...
        if(Math.abs(sourceCardValue - destinationCardValue) == 1){
            legalMove = true;
                ...
                (sourceCardValue == 14 && destinationCardValue == 2)){
            legalMove = true;
        }
        else {
            legalMove = false;
    ...
    public boolean DrawCard(int playerIndex){
        ...
        else if(playerHands.get(playerIndex).size() > 4){
            success = false;
        }
        else{
